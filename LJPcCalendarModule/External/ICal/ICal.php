...
namespace Modules\LJPcCalendarModule\External\ICal;
...
class ICal {
	...
	public function __construct( $files = false, array $options = [] ) {
		...
		$this->windowMinTimestamp = is_null( $this->filterDaysBefore )
			? $php_int_min : ( new DateTime( 'now' ) )->sub( new DateInterval( 'P' . $this->filterDaysBefore . 'D' ) )->getTimestamp();
		$this->windowMaxTimestamp = is_null( $this->filterDaysAfter )
			? PHP_INT_MAX : ( new DateTime( 'now' ) )->add( new DateInterval( 'P' . $this->filterDaysAfter . 'D' ) )->getTimestamp();
				...
				if ( is_array( $value ) ) {
					...
					if ( $keyword === 'EXDATE' ) {
						if ( trim( $value ) === $value ) {
							...
							$value                                              =
								explode( ',', implode( ',', $this->cal[ $key1 ][ $key2 ]["{$keyword}_array"][1] ) . trim( $value ) );
			...
			foreach ( $events as $key => $event ) {
				...
				if ( (bool) array_product( $checks ) ) {
					...
					if ( ( $alteredEventKey = array_search( $eventDtstartUnix, $this->alteredRecurrenceInstances[ $event['UID'] ] ) ) !== false ) {
						...
						$alteredEvent                                                       =
							array_replace_recursive( $events[ $key ], $events[ $alteredEventKey ] );
